...
tokens.post = function(context,httpResponse){
  ...
  let {id,extend,email,password} = inputSanitation(payload);
        ...
        if(password == userData.password){
          ...
          _data.create(TOKEN_TABLE_NAME,tokenId,tokenObject,function(err){
            if(!err){
              ...
              httpResponse(500,{'Error' : 'Could not create the new token'});
            }
          });
        } else {
          httpResponse(400,{'Error' : 'Password did not match the specified user\'s stored password'});
        }
      } else {
        httpResponse(400,{'Error' : 'Could not find the specified user.'});
      }
    });
  } else {
    httpResponse(400,{'Error' : 'Missing required field(s).'})
  }
};
...
tokens.get = function(context,httpResponse){
  ...
  if(id){
    ...
    httpResponse(400,{'Error' : 'Missing required field, or field invalid'})
  }
};
        ...
        if(tokenData.expires > Date.now()){
          ...
          _data.update('tokens',id,tokenData,function(err){
            if(!err){
              ...
              httpResponse(500,{'Error' : 'Could not update the token\'s expiration.'});
            }
          });
        } else {
          httpResponse(400,{"Error" : "The token has already expired, and cannot be extended."});
        }
      } else {
        httpResponse(400,{'Error' : 'Specified user does not exist.'});
      }
    });
  } else {
    httpResponse(400,{"Error": "Missing required field(s) or field(s) are invalid."});
  }
};
      ...
      if(!err && tokenData){
        ...
        _data.delete(TOKEN_TABLE_NAME,id,function(err){
          if(!err){
            ...
            httpResponse(500,{'Error' : 'Could not delete the specified token'});
          }
        });
      } else {
        httpResponse(400,{'Error' : 'Could not find the specified token.'});
      }
    });
  } else {
    httpResponse(400,{'Error' : 'Missing required field'})
  }
};
